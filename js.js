...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Initialize the holographic profile effect
...
// Enhanced profile holographic effect
function initHolographicEffect() {
    ...
    // Create random holographic particles
    for (let i = 0; i < 30; i++) {
        const particle = document.createElement('div');
        particle.className = 'particle';
        ...
        particle.style.setProperty('--z', `${zPos}px`);
        
        // Random size, animation delay and duration
        ...
        const hue = Math.random() > 0.5 ? 328 : 340; // Pink/magenta hues
        particle.style.boxShadow = `0 0 ${4 + Math.random() * 8}px hsla(${hue}, 100%, 70%, 0.8)`;
        particle.style.background = `hsla(${hue}, 100%, 70%, 0.8)`;
        
        particlesContainer.appendChild(particle);
    }
    
    // Create the scanning effect
    const scanner = document.querySelector('.profile-scanner');
    if (scanner) {
        scanner.style.animation = 'scan-light 5s infinite ease-in-out';
    }
    ...
    profileContainer.addEventListener('mousemove', (e) => {
        ...
        if (profileCard && profileImg) {
            ...
            profileCard.style.transform = `perspective(1500px) rotateX(${rotX}deg) rotateY(${rotY}deg)`;
            
            // Move the image slightly to create parallax effect
            ...
            profileImg.style.boxShadow = `
                ...
                ${-x/30}px ${-y/30}px 75px rgba(255, 105, 180, 0.3)
            `;
        }
    });
